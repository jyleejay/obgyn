...
# import files
CIN1 <- read.csv("CINProgression/data/CIN1_tidy_1206.csv", stringsAsFactors = F, fileEncoding = "euc-kr") %>% 
  mutate(CIN1_gr = 1) # 477

CIN2 <- read.csv("CINProgression/data/CIN2_tidy_1206.csv", stringsAsFactors = F, fileEncoding = "euc-kr") %>% 
  mutate(CIN2_gr = 1) # 91
...
fu_tb <- tb.1 %>%
  ...
  as_gt() %>%
  gt::tab_header(
    title = gt::md("**Baseline characteristics**"))
...
CINdf.1 <- entire_CIN %>%
  ...
  mutate(event = factor(event, levels = c(0, 1, 2), labels = c("Persistent", "Progression", "Regression")),
         ...
         time = time,
         age_ca = factor(age_ca, levels = c("1", "2", "3", "4"), labels = c("20s", "30s", "40s", "over 50s")),
         hpvNum = factor(hpvNum, levels = c("lrNum", "Neg", "hrNum", "Unknown"), labels = c("Low risk or Negative", "Low risk or Negative", "High risk", "Unknown")),
         h_16 = factor(h_16, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         h_18 = factor(h_18, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         h_52 = factor(h_52, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         h_58 = factor(h_58, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         h_31 = factor(h_31, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         h_33 = factor(h_33, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         h_45 = factor(h_45, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         h_35 = factor(h_35, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         h_39 = factor(h_39, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         h_51 = factor(h_51, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         h_56 = factor(h_56, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         h_59 = factor(h_59, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         h_66 = factor(h_66, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         h_68 = factor(h_68, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         multi_inf = factor(multi_inf, levels = c(0, 1, "Unknown"), labels = c("No", "Yes", "Unknown")),
         vacc_c = factor(vacc_c, levels = c(2, 1, 0), labels = c("No", "Yes", "Unknown")),
...
####### using cuminc
# entire group
entire_ci_plot <- cuminc(Surv(time, event) ~ initial_st, data = CINdf.1, multiple_panels=TRUE) %>% 
  ggcuminc(outcome = c("Progression", "Regression"), size = 1) +
  scale_x_continuous(breaks = seq(0, 365*4, by = 365), limits = c(0, 365*4)) +
  add_risktable(
    risktable_stats = c("n.risk", "cum.event"),
    stats_label = c("Numbers at Risk", "Cum. Events")) +
  labs(
    x = "Time (Days)"
  ) +
  theme(legend.position = c(0.1, 0.85)) +
  annotate("text", x = 28, y = 0.29, label = paste(cin1_1, "%"))
...
cin1_intersect_prog <- with(approx(cin1_plot_prog$time, cin1_plot_prog$estimate, xout = CIN1_mediantime$prog),
                            data.frame(x1 = c(-Inf, x, x), y1 = c(y, y, -Inf)))

cin1_intersect_reg <- with(approx(cin1_plot_reg$time, cin1_plot_reg$estimate, xout = CIN1_mediantime$reg),
                           data.frame(x1 = c(-Inf, x, x), y1 = c(y, y, -Inf)))
...
cin1_plot <- cin1_plot_base +
  ...
  annotate("text", x = 1460, y = 0.095, label = value_cin1_prog, fontface = 2) +
  annotate("text", x = 1460, y = 0.35, label = value_cin1_reg, fontface = 2) +
  annotate("text", x = CIN1_mediantime$prog, y = cin1_intersect_prog$y1[1] + 0.045, label = glue("Progression\nMedian time\n  {CIN1_mediantime$prog} days", .trim = F)) +
  annotate("text", x = CIN1_mediantime$reg, y = cin1_intersect_reg$y1[1] + 0.045, label = glue("Regression\nMedian time\n  {CIN1_mediantime$reg} days", .trim = F))
...
cin2_intersect_prog <- with(approx(cin2_plot_prog$time, cin2_plot_prog$estimate, xout = CIN2_mediantime$prog),
                            data.frame(x1 = c(-Inf, x, x), y1 = c(y, y, -Inf)))

cin2_intersect_reg <- with(approx(cin2_plot_reg$time, cin2_plot_reg$estimate, xout = CIN2_mediantime$reg),
                           data.frame(x1 = c(-Inf, x, x), y1 = c(y, y, -Inf)))
...
cin2_plot <- cin2_plot_base +
  ...
  annotate("text", x = CIN2_mediantime$prog, y = cin2_intersect_prog$y1[1] + 0.045, label = glue("Progression\nMedian time\n  {CIN2_mediantime$prog} days", .trim = F)) +
  annotate("text", x = CIN2_mediantime$reg, y = cin2_intersect_reg$y1[1] + 0.045, label = glue("Regression\nMedian time\n  {CIN2_mediantime$reg} days", .trim = F))
...
uni_tab1 <-
  ...
  bold_p(t = 0.05) %>% 
  italicize_levels() %>%
  modify_caption("**CIN Univariate Regression**")
...
multi_tab1 <-
  ...
  bold_p(t = 0.05) %>% 
  italicize_levels() %>%
  modify_caption("**CIN Multivariate Regression**")
...
###### fine-gray test for p-value for 31, 39, 59, 68
fg_tab1 <- 
  ...
  remove_row_type(type = "level", level_value = "Yes") %>% 
  remove_row_type(type = "level", level_value = "No") %>% 
  remove_row_type(type = "level", level_value = "Unknown") %>% 
  ...
  modify_table_styling(
    ...
    missing_symbol = "NA",
    text_format = "italic",
    footnote = "No observation when event occured")
...
fg_tab4 <- 
  cuminc(Surv(time, event) ~ h_68, data = CINdf.1) %>%
  tbl_cuminc(label = "HPV high-risk type: 68") %>%
  bold_labels() %>% 
  add_n(location = "label") %>% 
  add_p() %>%
  bold_p(t = 0.05) %>% 
  ...
  modify_table_styling(
    columns = p.value,
    rows = variable %in% c("h_68"),
    footnote = "Gray's test")

stacked_tbl <- tbl_stack(list(multi_tab1, multi_tab2, multi_tab3, multi_tab4, fg_tab1, multi_tab5, multi_tab6, fg_tab2, multi_tab7, multi_tab8, multi_tab9, multi_tab10, multi_tab11, fg_tab3, multi_tab12, fg_tab4)) %>% 
  modify_table_styling(
    columns = c(estimate, ci),
    rows = variable %in% c("h_39", "h_59", "h_68"),
    missing_symbol = "-",
    text_format = "italic")
